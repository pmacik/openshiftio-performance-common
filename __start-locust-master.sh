...
if [ -f $LOCUST_FILE ]; then
	...
	CMD="nohup locust -f $JOB_BASE_NAME-$BUILD_NUMBER-locustfile.py --master --no-web --print-stats --no-reset-stats --expect-slaves=$SLAVES --clients=$USERS --hatch-rate=$USER_HATCH_RATE --csv=$LOG_DIR/csv/$JOB_BASE_NAME-$BUILD_NUMBER-report 2>$LOG_DIR/$JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log > $LOG_DIR/$JOB_BASE_NAME-$BUILD_NUMBER-locust-master.log &";
