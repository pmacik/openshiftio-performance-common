...
REPORT_REQUESTS="$LOG_DIR/csv/$INPUT-report_requests.csv"

if [ `cat $REPORT_REQUESTS | grep -F "$ENDPOINT" | wc -l` -lt 1 ]; then
	exit 1
fi

VALUES=(`cat $REPORT_REQUESTS | grep -F "$ENDPOINT" | cut -d ',' -f 3-10 | tr ',' ' '`)
